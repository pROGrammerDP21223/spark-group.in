if (!window.__enquiryFormScriptLoaded) {
    ...
    var API_BASE_URL = (typeof BACKEND_API_URL !== 'undefined' ? BACKEND_API_URL : window.location.origin).replace(/\/+$/, '');
    var API_KEY_VALUE = (typeof API_KEY !== 'undefined' ? API_KEY : '').trim();
    var FILE_UPLOAD_API_BASE = (typeof FILE_UPLOAD_API_URL !== 'undefined' ? FILE_UPLOAD_API_URL : '').replace(/\/+$/, '');
    ...
    var captchaBox = document.getElementById('captchaImage');
    var captchaAnswerInput = document.getElementById('captcha_text');
    var captchaIdInput = document.getElementById('captcha_id');
    var csrfInput = document.getElementById('csrf_token');
    var honeypotInput = document.getElementById('website_url');
    var fileInput = document.getElementById('file_upload');
    var filePreview = document.getElementById('filePreview');
    var fileName = document.getElementById('fileName');
    var removeFileBtn = document.getElementById('removeFile');
    var loaderOverlay = document.getElementById('formLoaderOverlay');
    var extraFieldIds = ['company_name', 'address', 'enquiry_details'];
    ...
    function fetchWithTimeout(url, options, timeoutMs) {
        ...
        var timeoutId = setTimeout(function () { controller.abort(); }, timeoutMs || REQUEST_TIMEOUT_MS);
        options = options || {};
    ...
    function uploadFile(file) {
        if (!FILE_UPLOAD_API_BASE) return Promise.reject(new Error('FILE_UPLOAD_API_URL is not configured'));
        var fd = new FormData();
        fd.append('file', file);
        return fetchWithTimeout(FILE_UPLOAD_API_BASE + '/api/upload', { method: 'POST', body: fd }, REQUEST_TIMEOUT_MS)
            .then(function (res) {
                return res.json().catch(function () { return {}; }).then(function (data) {
                    if (!res.ok || !data.success || !data.data || !data.data.file_link) {
    ...
    function validateInput() {
        ...
        email = email.trim();
        var captchaAnswer = (captchaAnswerInput && captchaAnswerInput.value ? captchaAnswerInput.value : '').trim();
    ...
    function buildPayload(csrfToken, fileLink) {
        ...
        extraFieldIds.forEach(function (id) {
            ...
            if (el && el.value && el.value.trim()) rawPayload[el.name || id] = el.value.trim();
        });
        var productIdInput = document.querySelector('input[name="product_id"]');
        var brandIdInput = document.querySelector('input[name="brand_id"]');
        if (productIdInput && productIdInput.value) rawPayload.product_id = productIdInput.value;
        if (brandIdInput && brandIdInput.value) rawPayload.brand_id = brandIdInput.value;
        ...
        if (Object.keys(rawPayload).length) payload.rawPayload = rawPayload;
        return payload;
    }
    ...
    function loadCaptchaChallenge() {
        ...
        return fetchWithTimeout(API_BASE_URL + '/api/enquiries/captcha-challenge', {
            method: 'GET',
            headers: { 'X-API-Key': API_KEY_VALUE }
        }, REQUEST_TIMEOUT_MS).then(function (response) {
            return response.json().catch(function () { return {}; }).then(function (data) {
                if (!response.ok || !data.data || !data.data.captchaId || !data.data.question) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (refreshCaptchaBtn) {
            refreshCaptchaBtn.addEventListener('click', function () {
                loadCaptchaChallenge().catch(function (error) {
        ...
        form.addEventListener('submit', function (e) {
            ...
            if (!API_KEY_VALUE || API_KEY_VALUE.indexOf('PASTE_') !== -1) return showMessage('API_KEY is not configured in form_config.js.', 'error');

            var validationError = validateInput();
            if (validationError) return showMessage(validationError, 'error');
            if (honeypotInput && honeypotInput.value) return showMessage('Enquiry submitted successfully!', 'success');
            ...
            Promise.resolve()
                .then(function () {
                    if (!captchaIdInput || !captchaIdInput.value) return loadCaptchaChallenge();
                })
                .then(function () {
                ...
                .then(function (fileLink) {
                    var payload = buildPayload(csrfToken, fileLink);
                    return fetchWithTimeout(API_BASE_URL + '/api/enquiries', {
                        method: 'POST',
                ...
                .then(function (response) {
                    return response.json().catch(function () { return {}; }).then(function (data) {
                        if (response.ok) {
                            showMessage('Enquiry submitted successfully! ID: ' + (data && data.data && data.data.id ? data.data.id : 'N/A'), 'success');
                            form.reset();
                            if (filePreview) filePreview.style.display = 'none';
                        } else {
                ...
                .catch(function (err) {
                    if (err && err.name === 'AbortError') showMessage('Request timed out. Please try again.', 'error');
                    else showMessage('Network/security error: ' + (err && err.message ? err.message : 'Could not submit enquiry.'), 'error');
                    return loadCaptchaChallenge().catch(function () {});
                })
                .finally(function () {
        ...
        loadCaptchaChallenge().catch(function (error) {
            showMessage('Captcha init failed: ' + (error && error.message ? error.message : 'Unknown error'), 'error');
        });
    });
}
...
async function uploadFile(file) {
    ...
    const res = await fetchWithTimeout(`${FILE_UPLOAD_API_BASE}/api/upload`, { method: 'POST', body: fd });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || !data.success || !data.data?.file_link) throw new Error(data.error || data.message || 'File upload failed');
    return data.data.file_link;
}
...
async function loadCaptchaChallenge() {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok || !data.data?.captchaId || !data.data?.question) throw new Error(data.message || `Captcha challenge failed (HTTP ${response.status})`);
    if (captchaIdInput) captchaIdInput.value = data.data.captchaId;
    if (captchaBox) captchaBox.textContent = data.data.question;
    if (captchaAnswerInput) captchaAnswerInput.value = '';
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    refreshCaptchaBtn?.addEventListener('click', async () => {
        try { await loadCaptchaChallenge(); } catch (error) { showMessage(error.message || 'Failed to refresh captcha.', 'error'); }
    });
    ...
    form.addEventListener('submit', async (e) => {
        ...
        hideMessage();
        if (!API_BASE_URL) return showMessage('API_BASE_URL is not configured.', 'error');
        if (!API_KEY_VALUE || API_KEY_VALUE.includes('PASTE_')) return showMessage('API_KEY is not configured in form_config.js.', 'error');
        const validationError = validateInput();
        if (validationError) return showMessage(validationError, 'error');
        if (honeypotInput?.value) return showMessage('Enquiry submitted successfully!', 'success');
        ...
        try {
            ...
            if (!captchaIdInput?.value) await loadCaptchaChallenge();
            let fileLink = '';
            if (fileInput?.files?.[0]) fileLink = await uploadFile(fileInput.files[0]);
            ...
            if (err?.name === 'AbortError') showMessage('Request timed out. Please try again.', 'error');
            else showMessage(`Network/security error: ${err?.message || 'Could not submit enquiry.'}`, 'error');
    ...
    try { await loadCaptchaChallenge(); }
    catch (error) { showMessage(`Captcha init failed: ${error.message || 'Unknown error'}`, 'error'); }
});

/**
 * Enquiry form logic (new flow only):
 * - Load captcha challenge from /api/enquiries/captcha-challenge
 * - Submit to /api/enquiries
 * - Optional file upload via FILE_UPLOAD_API_URL and store file_link in rawPayload
 */
...
async function uploadFile(file) {
    if (!FILE_UPLOAD_API_BASE) {
        throw new Error('FILE_UPLOAD_API_URL is not configured');
    }

    const fd = new FormData();
    fd.append('file', file);

    const res = await fetchWithTimeout(`${FILE_UPLOAD_API_BASE}/api/upload`, {
        method: 'POST',
        body: fd
    });
    const data = await res.json().catch(() => ({}));
...
function buildPayload(csrfToken, fileLink) {
    ...
    extraFieldIds.forEach((id) => {
        ...
        if (el && el.value && el.value.trim()) {
            rawPayload[el.name || id] = el.value.trim();
        }
    });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!API_BASE_URL) {
            showMessage('API_BASE_URL is not configured.', 'error');
            return;
        }
        if (!API_KEY_VALUE || API_KEY_VALUE.indexOf('PASTE_') !== -1) {
            showMessage('API_KEY is not configured in form_config.js.', 'error');
            return;
        }
